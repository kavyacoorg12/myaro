...
export const PostCard: React.FC<PostCardProps> = ({
  ...
  const [showLikes, setShowLikes] = useState(false); // ← NEW
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col leading-tight">
              <span
                onClick={handleProfileClick}
                className="text-sm font-semibold text-gray-900 flex items-center gap-1 cursor-pointer"
              >
                {post.user.userName}
        ...
        <div className="px-4 pt-3 pb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              {/* Heart button — toggles like only */}
              <button
                onClick={handleLike}
                ...
                <Heart
                  className={`w-5 h-5 transition-colors ${
                    liked
                      ...
                      : "text-gray-700 group-hover:text-rose-400"
                  }`}
                />
              </button>

              {/* Likes count — opens LikeListModal */}
              {localLikes > 0 && (
                <button
                  onClick={() => setShowLikes(true)}
                  className="text-sm font-medium text-gray-700 hover:underline -ml-2"
                >
                  {localLikes}
                </button>
              )}
      ...
      {/* LikeListModal — rendered outside the card div so z-index stacks correctly */}
      {showLikes && (
        <LikeListModal
          postId={post.id}
