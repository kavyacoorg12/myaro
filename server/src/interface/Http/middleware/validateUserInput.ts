...
export function validateUserInput(
  ...
  if (userName.length < 6 || userName.length > 40) {
    return res
      .status(400)
      .json({ error: "Username must be between 6 and 40 characters" });
  }

  if (typeof fullName !== "string") {
    return res
      .status(400)
      .json({ error: "full name must contain characters (a-zA-Z)" });
  }
  if (fullName.trim().length === 0) {
    return res
      .status(400)
      .json({ error: "full name can't be Empty or Whitespace " });
  }

  if (fullName.length > 20) {
    return res
      .status(400)
      .json({ error: "full name should not exceed 20 characters" });
  }
  const regexPassword =
    /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$/;
...
export function ValidateComment(
  ...
  if (postId && beauticianId) {
    return res.status(HttpStatus.BAD_REQUEST).json({
      error: "Comment can't have both postId and beauticianId",
    });
  }

  if (!postId && !beauticianId) {
    return res.status(HttpStatus.BAD_REQUEST).json({
      error: "Comment must have either postId or beauticianId",
    });
  }
  ...
  if (text.trim().length > 400) {
    return res.status(HttpStatus.BAD_REQUEST).json({
      error: "Comment must be below 400 characters",
    });
  }
